                    ...
                    <div class="col-md-6">
                        <nav aria-label="Page navigation" class="pull-right">
                            <ul class="pagination pagination-sm" style="margin: 0px;">
                                <li>
                                    <a href="${pageContext.request.contextPath}/book-type/bookInfo?pageIndex=1&pageSize">首页</a>
                                </li>
                                <%--第一页时不可点击上一页--%>
                                ...
                                <c:if test="${pageInfo.pageNum!=1}">
                                    <li>
                                        <a href="${pageContext.request.contextPath}/book-type/bookInfo?pageIndex=${pageInfo.pageNum-1}&pageSize"
                                ...
                                <c:forEach items="${pageInfo.navigatepageNums}" step="1" var="itemPage">
                                    ...
                                    <c:if test="${pageInfo.pageNum == itemPage}">
                                        <li class="active">
                                            <a
                                                    href="${pageContext.request.contextPath}/book-type/bookInfo?pageIndex=${itemPage}&pageSize">${itemPage}</a>
                                        </li>
                                    </c:if>
                                    <%--非当前页 不激活--%>
                                    <c:if test="${pageInfo.pageNum != itemPage}">
                                ...
                                <li><a
                                        href="${pageContext.request.contextPath}/book-type/bookInfo?pageIndex=${pageInfo.pages}&pageSize">尾页</a>
                                </li>
                            </ul>
                        </nav>
                    </div>
...
<div class="popOut">
    ...
    <form method="post" action="/book-type/typeAdd">
    <table>
        <caption>添加图书类型</caption>
        <tr>
            <td width="120">图书类型：</td>
            <td><input type="number" class="inp" placeholder="图书类型" name="typeId"/></td>
        </tr>
        <tr>
            <td>类型名：</td>
            <td><input type="text" class="inp" placeholder="类型名" name="typeName"/></td>
        </tr>
        <tr>
            <td colspan="2"><input type="submit" class="login"/>
                添加
            </td>
    ...
    $(function () {
        ...
        $(".prePage").click(function () {
            if (curPage > 1) {
                ...
                $(this).attr("href", "${pageContext.request.contextPath}/book-type/bookInfo?pageIndex=" + pageNo + "&pageSize");
            }
        });
        //下一页
...
<script>
    ...
    var results = "${result}";
    console.log(results)
    if (results === "returnSuccess") {
        $('.alert').html('归还成功').addClass('alert-success').show().delay(2500).fadeOut();
    }
    if (results === "deleteSuccess") {
        $('.alert').html('删除成功').addClass('alert-success').show().delay(2500).fadeOut();
    }
    if (results === "no") {
